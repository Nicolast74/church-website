...
const LocationCard: React.FC<LocationCardProps> = ({ imageSrc, title, description, linkHref }) => (
  <div className="w-full bg-white rounded-[2.5rem] border border-slate-100 p-4 hover:shadow-[0_32px_64px_-15px_rgba(0,0,0,0.1)] transition-all duration-500 group">
    ...
    <div className="px-4 pb-4">
      ...
      <Link href={linkHref} className="inline-block w-full py-4 bg-slate-50 text-slate-900 text-[10px] font-black tracking-widest rounded-2xl hover:bg-slate-900 hover:text-white transition-all text-center">
        DIREKSI LOKASI
      </Link>
    </div>
  </div>
